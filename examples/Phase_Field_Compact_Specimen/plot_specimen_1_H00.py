r"""
.. _ref_phase_field_compact_specimen_1_H00:

Specimen 1
----------

This example demonstrates the simulation of a compact specimen with a phase-field fracture model. The specimen geometry corresponds to a configuration where the parameter $H$ is set to 0.0 mm. 

The mesh used for this simulation is generated using Gmsh and is based on the geometry described in :ref:`ref_example_geo_specimen_1_H00`. This geometry includes predefined regions and boundary markers essential for applying boundary conditions and loads during the simulation.
...
Data = Input(E=211.0,   
             ...
             results_folder_name="results_specimen_1_H00")
...
# which is provided in the reference :ref:`example_geo_specimen_1_H00`.
msh_file = os.path.join("../GmshGeoFiles/Compact_specimen/specimen_1_H00.msh")  # Path to the mesh file
...
if measure:
    ...
    results_a_measured = np.loadtxt("../Phase_Field_Compact_Specimen/results_specimen_1_H00/crack_measurement/interpolated_step_time_crack_length.txt", skiprows=1)
...
pv.start_xvfb()
file_vtu = pv.read(os.path.join(Data.results_folder_name, "paraview-solutions_vtu", "phasefieldx_p0_000162.vtu"))
